...
class NowcastMetrics(Callback):
    ...
    def on_validation_batch_end(
        ...
        if hasattr(pl_module, "mode"):
            if "heteroscedastic" in pl_module.mode:
                y_hat = y_hat[...,0] # take only pred, not variance
        # transformed -> dbz
        to_dbz = trainer.datamodule.valid_dataset.output_to_storage_conversion
        # dbz -> mm/h
        to_mmh = trainer.datamodule.valid_dataset.dbz_to_mmh
        self.valid_scores.update(to_mmh(to_dbz(y_hat)), to_mmh(to_dbz(y)))
    ...
    def _epoch_end(self, stage, trainer, pl_module):
        ...
        pl_module.log(f"mean_{stage}_ETS", cat_df.mean().ETS)
        ...
        pl_module.log(f"mean_{stage}_MAE", cont_df.mean().MAE)
